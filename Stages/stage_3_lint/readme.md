...
4. [Stage 2 — Framework-Specific Build Steps](#-stage-2--framework-specific-build-steps)
5. [Stage 3 — Lint Pipeline](#-stage-3--lint-pipeline)
6. [Merge & Official CI Build](#-merge--official-ci-build)
7. [Promotion Ladder — DEV → QA → STAGING](#-promotion-ladder--dev--qa--staging)
8. [Canary Release to Production](#-canary-release-to-production)
...
> **Purpose:** Catch unwanted code, style violations, unused variables, missing keys, and type errors **before** code reaches review or production.

This stage runs **7 parallel linters** — one per file type — all feeding into a single **Lint Gate** decision point. If the gate says **NO**, the PR is **BLOCKED**. If **YES**, the PR **PASSES**.
...
graph TD
    ...
    style JS fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style PY fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style YAML fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style JSON fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style CSS fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style JAVA fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style MD fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    ...
    style ESLINT fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style PYLINT fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style YAMLLINT fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style JSONLINT fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style CSSLINT fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style CHECKSTYLE fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    style MDLINT fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    ...
    style NO fill:#ffffff,stroke:#333,stroke-width:2px,color:#000
    style YES fill:#ffffff,stroke:#333,stroke-width:2px,color:#000
    style BLOCK fill:#d5e8d4,stroke:#82b366,stroke-width:2px,color:#000
    ...
    %% Dashed Styling
    linkStyle 0,1,2,3,4,5,6 stroke:#82b366,stroke-width:2px,stroke-dasharray: 5 5
    linkStyle 14,15,16,17,18,19,20 stroke:#82b366,stroke-width:2px,stroke-dasharray: 5 5
    linkStyle 21,22,23,24 stroke:#9673a6,stroke-width:2px,stroke-dasharray: 5 5
```

---

### 📋 Linter Matrix — One Per File Type

| File Type | Linter | What It Catches |
|-----------|--------|-----------------|
| **JS / TS / TSX** | ESLint + Biome | Unsaved vars, unused imports, missing keys, hook rules, type errors |
| **PYTHON** | pylint + flake8 | PEP8 violations, unused imports, undefined names |
| **YAML** | yamllint | Indentation, duplicate keys, invalid syntax |
| **JSON** | jsonlint | Malformed JSON, trailing commas, duplicate keys |
| **CSS / SCSS** | csslint | Invalid properties, duplicate selectors, empty rules |
| **JAVA** | checkstyle | Naming conventions, import order, whitespace |
| **MD** | markdownlint | Heading levels, list indentation, line length |

---

### 🔍 What the Lint Gate Checks

The **LINT-GATE** aggregates output from all 7 linters and looks for:

| Category | Description |
|----------|-------------|
| **UNSAVED VARS** | Variables declared but never used |
| **MISSING KEYS** | React list items without `key` prop |
| **HOOK RULES** | React Hooks called conditionally or outside components |
| **TYPE ERRORS** | TypeScript type mismatches (via `tsc --noEmit`) |

> ❌ If **any** of these are found → **BLOCK**  
> ✅ If **all** are clean → **PASS**
...
|---------|---------|--------|
| **NO** | Lint errors found | PR is **BLOCKED** — developer must fix |
| **YES** | No lint errors | PR **PASSES** to next stage (Tests / Merge) |
...
### 🎯 Manager-Friendly Summary (Lint Stage)

| Question | Answer |
|----------|--------|
| **Why 7 linters?** | Each file type has different syntax rules — one linter can't handle all |
| **Why a central gate?** | Single decision point — no ambiguity on pass/fail |
| **What blocks the PR?** | Any error (unsaved vars, missing keys, hook rules, type errors) |
| **What doesn't block?** | Warnings are logged but allowed |
| **How long does it take?** | ~45 seconds total (all linters run in parallel) |
| **Can developers fix locally?** | Yes — `npm run lint --fix` auto-fixes most issues |

---

### 📁 Lint Config Files in Repo

```
ecom-frontend/
├── .eslintrc.json          # JS/TS/TSX rules
├── biome.json              # Biome formatter + linter
├── .pylintrc               # Python linting
├── .flake8                 # Python style
├── .yamllint               # YAML rules
├── .jsonlintrc             # JSON rules
...
### 🛠️ Troubleshooting Lint Failures

| Error | Cause | Fix |
|-------|-------|-----|
| `no-unused-vars` | Variable declared but never used | Remove it or prefix with `_` |
| `react/jsx-key` | Missing `key` prop in list | Add `key={item.id}` |
| `react-hooks/rules-of-hooks` | Hook called conditionally | Move hook to top level |
| `@typescript-eslint/no-explicit-any` | `any` type used | Use proper type or `unknown` |
| `yamllint: line too long` | YAML line > 80 chars | Wrap or split |
| `markdownlint: MD013` | Markdown line too long | Break into multiple lines |

> 💡 **Pro tip:** Run `npm run lint --fix` before every commit. Add it to a pre-commit hook with Husky.

---

### 🔗 Integration with Other Stages

- **Stage 2 (Build):** Runs in parallel with Lint — no dependency.
- **Stage 4 (Tests):** Only runs if Lint Gate passes.
- **Quality Gate:** Lint result is **one of four** required green checks.
- **Merge:** Blocked if Lint Gate returns NO.
