PR-CI-Pipeline — Developer **README** Author: Shubham Tripathi Repository: ecom-frontend (Next.js 14 · App Router · TypeScript) Last Updated: **2026**-09-18 Audience: Frontend Engineers, Reviewers, Tech Leads, QA, DevOps

Table of Contents The Trigger — Ticket → PR → Pipeline
...
The flow, at a glance Ticket → Branch → Code → Push → PR → Pipeline fires → Gates pass → Merge → Deploy → Live.

That's it. Let me walk through each part.

Before you push Do this every time. It saves CI runs and saves you waiting. bash git checkout main git pull origin main git checkout -b feature/coupon-checkout

write your code npm ci npm run lint npx tsc --noEmit npm test npm run build

only push when everything is green git add . git commit -m *feat(coupon): add coupon input to checkout* git push origin feature/coupon-checkout If it fails locally, it'll fail in CI too. Fix it before pushing.

Opening the PR When you open the PR, two things happen in parallel: A human reviewer looks at your code
...
The pipeline runs four jobs in parallel Build — compiles the app (~90 sec) Lint — checks code style (~45 sec)
...
Build stage Runs npm ci then npm run build. Output depends on the framework: React (**CRA**) → build/
...
Lint stage Every file type gets its own linter: .js .ts .tsx → ESLint + Biome
...
Security scans Six layers run in parallel. Each one targets something different: Secret scan (gitleaks) — catches hardcoded **API** keys
...
Tests Framework-specific runners: React → Jest + React Testing Library
...
Quality Gate This is the decision point. All four jobs must pass: Build ✅
